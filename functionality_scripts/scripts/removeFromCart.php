...
if($_SERVER['REQUEST_METHOD']=='POST'){
    if(isset($_POST['u_email']) && isset($_POST['p_recid']) && isset($_POST['pr_recid'])) {
        ...
        */
        $db_response = $db->removeFromCart($_POST['u_email'], $_POST['p_recid'], $_POST['pr_recid']);
        if($db_response == 1){
          $response['error'] = false;
          $response['message'] = "Item removed from shopping cart!";
        } else if($db_response == -1){
          $response['error'] = true;
          $response['message'] = "Item couldnt be removed!";
        } else {
          $response['error'] = true;
